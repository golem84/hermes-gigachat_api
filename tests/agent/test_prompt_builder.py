...
class TestGuidanceConstants:
    def test_memory_guidance_discourages_task_logs(self):
        ...
        assert "memory(action='read')" in MEMORY_GUIDANCE
        assert "previous sessions" in MEMORY_GUIDANCE
        assert "like a diary" not in MEMORY_GUIDANCE
        assert ">80%" not in MEMORY_GUIDANCE

    def test_session_search_guidance_is_simple_cross_session_recall(self):
        assert "previous sessions" in SESSION_SEARCH_GUIDANCE
        assert "cross-session recall" in SESSION_SEARCH_GUIDANCE
        assert "built-in memory" in SESSION_SEARCH_GUIDANCE
